...
const HealthCareAdmin = () => {
    ...
    const {
        ...
        queryFn: async () => {
            try {
                const response = await axiosPublic.get("/healthcare");
                return response.data;
            } catch (error) {
                toast.error("Failed to fetch healthcare appointments");
                throw error;
            }
        },
    });

    const filteredAppointments = healthcares.filter((appointment) => {
        const matchesSearch =
            appointment.petName
                ?.toLowerCase()
    ...
    const handleStatusUpdate = async (id, newStatus) => {
        ...
        try {
            await axiosPublic.patch(`/healthcare/${id}`, { status: newStatus });
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            ...
            try {
                await axiosPublic.delete(`/healthcare/${id}`);
                toast.update(toastId, {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (selectedAppointment?._id) {
                ...
                await axiosPublic.put(`/healthcare/${_id}`, appointmentData);
                toast.update(toastId, {
            ...
            <header className="bg-gradient-to-r from-[#FF7350] to-[#FF552A] shadow-lg">
                <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
                    ...
                    <p className="mt-2 text-white opacity-90">
                        Manage all pet health appointments
                    </p>
                </div>
            </header>
